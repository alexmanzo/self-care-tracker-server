...
router.post('/', jsonParser, (req, res) => {
  const requiredFields = ['username', 'password', 'admin', 'email', 'firstName', 'lastName']
  ...
  const explicityTrimmedFields = ['username', 'password', 'email', 'firstName', 'lastName'];
  ...
  return User.find({username}, {email})
    ...
    .then(count => {
      if (count > 0) {
        // There is an existing user with the same username or email
